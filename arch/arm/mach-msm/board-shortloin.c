...
static struct {
	...
	{OPAL_3G_DVT,      "opal-6thbuild-3G"},
	{OPAL_3G_PVT,      "opal-7thbuild-3G"},
...
static int config_camera_on_gpios_vx6953(void)
{
	...
	if ( !gpios_vreg_shortloin_cam_on ) {
		...
		rc = pm8058_gpio_config(camera_pwdn_cfg.gpio,
					&camera_pwdn_cfg.cfg);
		if (rc < 0) {
				pr_err("%s pmic gpio 9 config failed 1\n",
						__func__);
				return rc;
			}
...
static void config_camera_off_gpios_vx6953(void)
{
	...
	struct pm8058_gpio_cfg camera_pwdn_cfg={
			/* GPIO_09  Opal 5M Camera StandBy Pin */
			8,
			{
				.direction	= PM_GPIO_DIR_OUT,
				.output_value	= 0,
	...
	if (gpios_vreg_shortloin_cam_on) {
		...
		rc = pm8058_gpio_config(camera_pwdn_cfg.gpio,
				  &camera_pwdn_cfg.cfg);
		if (rc < 0) {
			pr_err("%s pmic gpio 9 config failed\n",
				  __func__);
			return;
		}
		if (IS_ERR_OR_NULL(votg_lvs0)) {
...
{
	if(board_type == OPAL_PROTO)
		gpio_set_value( SHORTLOIN_WEBCAM_FLASH, enable );
	else if(board_type == OPAL_EVT1)
...
static int shortloin_pins_wifi [] = {
...
static int shortloin_pins_evt1_3g [] = {
	[SHORTLOIN_A6_0_TCK_PIN] = SHORTLOIN_A6_0_TCK_EVT1,
	[SHORTLOIN_A6_0_WAKEUP_PIN] = SHORTLOIN_A6_0_WAKEUP,
	[SHORTLOIN_A6_0_TDIO_PIN] = SHORTLOIN_A6_0_TDIO,
	[SHORTLOIN_A6_0_MSM_IRQ_PIN] = SHORTLOIN_A6_0_MSM_IRQ_EVT1,
...
static void __init msm8x60_init(struct msm_board_data *board_data)
{
	...
	if(boardtype_is_3g())
	{
		if (board_type >= OPAL_3G_EVT2) {
			pin_table = &shortloin_pins_3g[0];
		} else {
				pin_table = &shortloin_pins_evt1_3g[0];
		}
	}
	else
	{
		pin_table = &shortloin_pins_wifi[0];
	}
