						...
						unsigned int cmd)
{
	...
	case KGSL_USER_MEM_TYPE_ASHMEM:
	{
		...
		if (!param.len)
			param.len = len;
/* PALM: Removing this check because we will map pointers to
 * memory out of one large pool. Due to the original allocation
 * of the pool being very large, this check incorrectly fails.
*/
/* 
