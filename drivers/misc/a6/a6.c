...
int32_t enq_a6_action_item(struct a6_device_state* state, struct a6_action_item* ai, bool enq_tail)
{
	...
	char buf_task_comm[TASK_COMM_LEN];
	ktime_t s_ti, e_ti;
	...
	do {
		get_task_comm(buf_task_comm, current);
		rc = wait_event_interruptible(ai->ai_waitq, ai->ai_complete);
		if (rc < 0) {
			if (!rate_limit) {
				printk(KERN_ERR "%s [task: %s]: wait on ai_complete interrupted; rc: %d\n",
					__func__, buf_task_comm, rc);
				rate_limit = true;
				s_ti = ktime_get();
			}
		}
	...
	if (rate_limit) {
		e_ti = ktime_get();
		printk(KERN_ERR "%s: interrupted task completed in %lld ms.\n", __func__,
			ktime_to_ms(ktime_sub(e_ti, s_ti)));
	}
